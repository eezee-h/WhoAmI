...
관리자는 로그인 후 공개 페이지에서 직접 내용을 수정할 수 있습니다.

- 헤더의 `편집 시작` / `편집 종료`로 보기 모드와 편집 모드를 전환합니다.
- `편집 종료`는 편집 UI만 끄며 로그인 상태는 유지합니다.
- `로그아웃`을 누르면 인증 정보가 지워지고 다음 편집 때 다시 로그인해야 합니다.
- 커스텀 카드 섹션은 화면에 보이는 이름과 URL에 사용할 이름을 따로 지정할 수 있습니다.
