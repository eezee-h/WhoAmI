...
회원가입 후 각 사용자는 `/{username}` 주소의 공개 페이지를 가지며, 로그인한 관리자는 화면에서 바로 프로필, 경력, 활동, 프로젝트, 연락처를 수정할 수 있습니다.
